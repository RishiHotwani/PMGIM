...
export default function AuthGateView({ onAuthSuccess }) {
  ...
  useEffect(() => {
    const initGis = () => {
      if (window.google?.accounts?.id) {
        try {
          const clientId = import.meta.env.VITE_GOOGLE_CLIENT_ID || '109827364512-gimcampus.apps.googleusercontent.com';
  ...
  const promptFallbackEmailAuth = () => {
    const email = prompt('Enter your Google Gmail address (e.g. student@gim.ac.in):');
    if (!email || !email.includes('@')) return;
    setLoading(true);

    // Generate valid test JWT credential payload for development environment
    ...
    loginWithGoogleToken(testJwtToken)
      .then((data) => {
        if (onAuthSuccess) onAuthSuccess(data.user);
      })
